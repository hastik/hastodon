...
class TracyDebugger extends \ProcessWire\WireData implements \ProcessWire\Module, \ProcessWire\ConfigurableModule {
   ...
   /**
    ...
    static public function getDefaultData() {
        return array(
            ...
            "requestInfoPanelSections" => array('moduleSettings', 'templateSettings', 'fieldSettings', 'pageInfo', 'redirectInfo', 'pagePermissions', 'languageInfo', 'templateInfo', 'fieldsListValues', 'serverRequest', 'inputGet', 'inputPost', 'inputCookie', 'session', 'editLinks'),
 ...
 require_once(\ProcessWire\wire('files')->compile(\ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger' . '/includes/CodeProcessor.php',array('includes'=>true,'namespace'=>true,'modules'=>false,'skipIfNamespace'=>false)));
                ...
                if($this->wire('input')->cookie->tracyModulesDisabled) {
                    ...
                    if(!file_exists($this->tracyCacheDir . $this->modulesDbBackupFilename)) {
                        ...
                        if(!$this->wire('files')->filePutContents($this->tracyCacheDir . 'restoremodules.php', $restoreModulesCode, LOCK_EX)) {
                            throw new \ProcessWire\WireException("Unable to write file: " . $this->tracyCacheDir . 'restoremodules.php');
                        }
                    }
        ...
        if(static::$allowedTracyUser === 'development') {
            ...
            $this->wire()->addHookBefore('Session::redirect', function($event) {
                static::$redirectInfo = \ProcessWire\Debug::backtrace()[0];
            });
 ...
 require_once(\ProcessWire\wire('files')->compile(\ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger' . '/includes/SlackLogger.php',array('includes'=>true,'namespace'=>true,'modules'=>false,'skipIfNamespace'=>false)));
                ...
                if($this->wire('input')->post->fileEditorFilePath) {
                    ...
                    if($this->wire('input')->post->tracyTestTemplateCode) {
                        if(!$this->wire('files')->filePutContents($this->tempTemplateFilename, $rawCode, LOCK_EX)) {
                            throw new \ProcessWire\WireException("Unable to write file: " . $this->tempTemplateFilename);
                        }
                    ...
                    if($this->wire('input')->post->tracyTestFileCode || $this->wire('input')->post->tracySaveFileCode || $this->wire('input')->post->tracyChangeTemplateCode) {
                        if($this->wire('input')->post->fileEditorFilePath != '' && strpos($this->wire('input')->post->fileEditorFilePath, '..') === false) {
                            ...
                            if(!$this->wire('files')->filePutContents($filePath, $rawCode, LOCK_EX)) {
                                throw new \ProcessWire\WireException("Unable to write file: " . $filePath);
                            }
 ...
 require_once(\ProcessWire\wire('files')->compile(\ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger' . '/includes/PwApiData.php',array('includes'=>true,'namespace'=>true,'modules'=>false,'skipIfNamespace'=>false)));
    ...
    public function getModuleConfigInputfields(array $data) {
        ...
        $f->notes = \ProcessWire\__('This results in a smaller, cleaner dump, but you may miss some information. You can override this with the `debugInfo => true/false` option when calling `d()`, `bd()`, etc. Note that this also affects the output in the Request Info panel\'s Field List & Values section.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 50;
        ...
        $f->notes = \ProcessWire\__('You can supply multiple addresses, one per line.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 34;
        ...
        $f->label = \ProcessWire\__('Slack Channel', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Post to this Slack channel when an error occurs.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 50;
        ...
        $f->label = \ProcessWire\__('Slack App Oauth Token', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 50;
